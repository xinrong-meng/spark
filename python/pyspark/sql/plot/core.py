...
class PySparkNumericPlotBase:
    @staticmethod
    def prepare_numeric_data(data: "DataFrame") -> Tuple["DataFrame", "DataFrame"]:
        numeric_columns = [
        ...
        if len(numeric_columns) == 0:
            raise PySparkTypeError(
                errorClass="PLOT_NO_NUMERIC_COLUMNS",
            )
...
class PySparkHistogramPlotBase(PySparkNumericPlotBase):
    @staticmethod
    def prepare_hist_data(data: "DataFrame", bins: int) -> Tuple["DataFrame", "np.ndarray"]:
        """
        Prepares the histogram data by computing bin intervals and selecting numeric columns.
        """
        data, numeric_data = PySparkNumericPlotBase.prepare_numeric_data(data)
        if isinstance(bins, int):
            # Computes boundaries for the columns in the DataFrame
    ...
    def get_bins(sdf: "DataFrame", bins: int) -> "np.ndarray":
        """
        Calculates bin boundaries for the numeric columns in the DataFrame.
        """
    ...
    @staticmethod
    def compute_hist(sdf: "DataFrame", bins: int):
